...
author:  |
  <a href="https://www.julianfludwig.com/" target="blank">Julian F. Ludwig</a> (Texas Tech University)
...
```{r index_file, eval=FALSE, echo=FALSE}
#========================================================================================
# Create index.html file by duplicating the preface.html file
#========================================================================================
# Set working directory to current file location
file_directory_name <- rstudioapi::getActiveDocumentContext()$path
file_directory <- dirname(file_directory_name)
file_name <- basename(file_directory_name)
setwd(file_directory) 
# Render R Markdown file
rmarkdown::render(file_name)
# Create index.html file as a copy of preface.html
dir.create("tmp")
file.copy("preface.html", "tmp")
file.rename(from = "tmp/preface.html", to = "tmp/index.html")
file.copy("tmp/index.html", "index.html", overwrite = TRUE)
unlink("tmp", recursive = TRUE)
```


```{r save_as_pdf, eval=FALSE, echo=FALSE}
#========================================================================================
# Create PDF file
#========================================================================================
# Set working directory to current file location
...
file_name <- basename(file_directory_name)
setwd(file_directory) 
# Render R Markdown file to PDF output
rmarkdown::render(file_name,  
                  encoding = 'UTF-8',
                  output_format = bookdown::pdf_book(),
                  params = list(pdf_output = TRUE))
```


```{r save_in_memory,eval=FALSE,echo=FALSE}
#========================================================================================
# Run and save variables in memory
#========================================================================================
# Get info
file_directory_name <- rstudioapi::getActiveDocumentContext()$path
file_directory <- dirname(file_directory_name)
file_name <- basename(file_directory_name)
# Run file
setwd(file_directory) # set working directory to current file location
rmarkdown::render(file_name)
```
...
```{r bookcover, echo = FALSE}
knitr::include_graphics("files/dreamstudio/1714417634_Economic Research Data Analysis_esrgan-v1-x2plus.png")
```
...
By the time you finish this book, you'll be ready to analyze a wide range of economic and financial data. Your learning progress will be assessed based on the completion of [DataCamp](https://www.datacamp.com){target="_blank"} courses and the production of three data reports, one for each module.
...
```{r photo_html_output, echo = FALSE, out.width = '70%', eval = !params$pdf_output}
knitr::include_graphics("files/jfl.png")
```


```{r photo_pdf_output, echo = FALSE, out.width = '20%', eval = params$pdf_output}
knitr::include_graphics("files/jfl.png")
```
...
[253 Holden Hall](http://www.ttu.edu/map/?b=Holden%20Hall){target="_blank"} \
Lubbock, TX 79409


Website: [www.julianfludwig.com](https://www.julianfludwig.com/){target="_blank"} \
Email: [julian.ludwig@ttu.edu](mailto:julian.ludwig@ttu.edu){target="_blank"} \
...
ECO 4300-D03: Data Analysis for Economic Research \
...
kable(date_table, booktabs = TRUE, 
    ...
    row_spec(1:nrow(date_table), font_size = 9)
```
...
For additional information about data tables and their powerful features, check out the [Intro to Data Table](https://cran.r-project.org/web/packages/data.table/vignettes/datatable-intro.html){target="_blank"} documentation and this [cheat sheet](https://s3.amazonaws.com/assets.datacamp.com/blog_assets/datatable_Cheat_Sheet_R.pdf){target="_blank"} for `data.table` functions. Furthermore, [DataCamp](https://learn.datacamp.com/){target="_blank"} provides several courses on `data.table`, such as:
...
To delve deeper into `xts` and `zoo` objects, consider reading the guides [Manipulating Time Series Data in R with xts & zoo](https://rstudio-pubs-static.s3.amazonaws.com/288218_117e183e74964557a5da4fc5902fc671.html){target="_blank"} and [Time Series in R: Quick Reference](https://rpubs.com/rmariscal/404513). Additionally, [DataCamp](https://learn.datacamp.com/){target="_blank"} provides in-depth courses on these topics: 
...
If you're working within the `tidyverse` environment, the R package [`tidyquant`](http://www.stats.bris.ac.uk/R/web/packages/tidyquant/vignettes/TQ02-quant-integrations-in-tidyquant.html){target="_blank"} offers seamless integration with `xts` and `zoo`. Lastly, this handy [cheat sheet](https://s3.amazonaws.com/assets.datacamp.com/blog_assets/xts_Cheat_Sheet_R.pdf){target="_blank"} provides a quick reference on `xts` and `zoo` functions.
...
The dataset contains 360 variables with coded column names such as `ics_inc31` or `pago_dk_all`. To understand the meaning of these columns, you can visit the same website [here](https://data.sca.isr.umich.edu/){target="_blank"} and click on [SURVEY INFORMATION](https://data.sca.isr.umich.edu/survey-info.php#){target="_blank"}. From there, select the [Time-Series Variable Codebook](https://data.sca.isr.umich.edu/fetchdoc.php?docid=45121){target="_blank"} which is a PDF document that provides detailed explanations for all the column names. By referring to this codebook, you can gain a better understanding of the variables and their corresponding meanings in the dataset.
...
1. Visit the website of the Federal Reserve Bank of Philadelphia by clicking [here](https://www.philadelphiafed.org/){target="_blank"}.
2. Navigate to [SURVEYS & DATA](https://www.philadelphiafed.org/surveys-and-data){target="_blank"}, select [Real-Time Data Research](https://www.philadelphiafed.org/surveys-and-data/real-time-data-research){target="_blank"}, choose [Tealbook (formerly Greenbook) Data Sets](https://www.philadelphiafed.org/surveys-and-data/real-time-data-research/greenbook){target="_blank"}, and finally select [Philadelphia Fed's Tealbook (formerly Greenbook) Data Set](https://www.philadelphiafed.org/surveys-and-data/real-time-data-research/philadelphia-data-set){target="_blank"}.
3. On the data page, download [Philadelphia Fed's Tealbook/Greenbook Data Set: Row Format](https://www.philadelphiafed.org/-/media/frbp/assets/surveys-and-data/greenbook-data/documentation/gbweb_row_format.xlsx?la=en&hash=ADEEBF4E1E339624C8C55B9EAB26B23F){target="_blank"}.
...
A [Web API](https://en.wikipedia.org/wiki/Web_API){target="_blank"} or Application Programming Interface is a tool that allows software applications to communicate with each other. It enables users to download or upload data from or to a server.

There are several economic databases that provide web APIs:

- International Monetary Fund (IMF) data can be downloaded in R directly through the [`imfr`](https://cran.r-project.org/web/packages/imfr/imfr.pdf){target="_blank"} package
- Bank for International Settlements (BIS) data can be accessed using the [`BIS`](https://cran.r-project.org/web/packages/BIS/vignettes/BIS.html){target="_blank"} package
- Organization for Economic Co-operation and Development (OECD) data is available through the [`OECD`](https://cran.r-project.org/web/packages/OECD/OECD.pdf){target="_blank"} package
...
To find the correct symbol for financial data from **Yahoo! Finance**, visit [finance.yahoo.com](https://finance.yahoo.com){target="_blank"} and search for the company of interest, e.g., Apple. The symbol is the series of letters inside parentheses. For example, for `Apple Inc. (AAPL)`, use `Symbols = "AAPL"` (and `src = "yahoo"`). Yahoo! Finance provides stock price data in various columns, such as `AAPL.Open`, `AAPL.High`, `AAPL.Low`, `AAPL.Close`, `AAPL.Volume`, and `AAPL.Adjusted`. These values represent the opening, highest, lowest, and closing prices for a given market day, the trade volume for that day, and the adjusted value, which accounts for stock splits. Typically, the `AAPL.Adjusted` column is of most interest.

For economic data from **FRED**, visit [fred.stlouisfed.org](https://fred.stlouisfed.org){target="_blank"}, and search for the data series of interest, e.g., unemployment rate. Click on one of the suggested data series. The symbol is the series of letters inside parentheses. For example, for `Unemployment Rate (UNRATE)`, use `Symbols = "UNRATE"` (and `src = "FRED"`).
...
[Quandl](https://data.nasdaq.com){target="_blank"} is a data service that provides access to many financial and economic data sets from various providers. It has been acquired by [Nasdaq Data Link](https://data.nasdaq.com){target="_blank"} and is now operating through the Nasdaq data website: [data.nasdaq.com](https://data.nasdaq.com){target="_blank"}. Some data sets require a paid subscription, while others can be accessed for free. Quandl provides an API that allows you to import data from a wide variety of languages, including Excel, Python, and R.

To use Quandl, first get a free Nasdaq Data Link API key at [data.nasdaq.com/sign-up](https://data.nasdaq.com/sign-up){target="_blank"}. After that, install and load the Quandl package, and provide the API key with the function `Quandl.api_key()`:
...
For more in-depth understanding and examples, consider taking DataCamp's [Introduction to Importing Data in R](https://learn.datacamp.com/courses/introduction-to-importing-data-in-r){target="_blank"} course.
...
To further enhance your knowledge and skills in working with data in R, consider the following [DataCamp](https://learn.datacamp.com/){target="_blank"} courses:

- [Importing and Managing Financial Data in R](https://learn.datacamp.com/courses/importing-and-managing-financial-data-in-r){target="_blank"}: Gain a solid understanding of how to import and manage financial data in R.
- [Intermediate Importing Data in R](https://learn.datacamp.com/courses/intermediate-importing-data-in-r){target="_blank"}: Build upon your data importing skills and learn how to handle larger and more complex datasets.
- [Working with Web Data in R](https://learn.datacamp.com/courses/working-with-web-data-in-r){target="_blank"}: Learn to process and analyze data from the web using R.
...
A key strength of R Markdown is the ability to incorporate R variables directly within the Markdown text. This enables a dynamic text where the values are updated as the variables change. You can accomplish this by using the `` `r "\u0060r variable\u0060"` `` syntax. Furthermore, you can format these numbers for enhanced readability.
...
````rmarkdown
The total amount is `r "\u0060r my_var\u0060"` USD.
````

The output will be: "The total amount is `r my_var` USD."

That's because when the R Markdown document is knitted, `` `r "\u0060r my_var\u0060"` `` will be replaced by the current value of `my_var` in your R environment, dynamically embedding the value of `my_var` into your text.
...
````rmarkdown
The total amount is `r "\u0060r my_var_formatted\u0060"` USD.
````
...
For an in-depth understanding of R Markdown, you may want to delve into [R Markdown: The Definitive Guide](https://bookdown.org/yihui/rmarkdown/){target="_blank"}, an extensive resource on the topic. Additionally, DataCamp's course [Reporting with R Markdown](https://learn.datacamp.com/courses/reporting-with-rmarkdown){target="_blank"} provides practical lessons on how to create compelling reports using this tool.
...
Perform a comparative analysis of the yield curves from your birth date and the most recent one available. Leverage the insights provided in Chapter \@ref(yield-curve) to interpret the shapes of these yield curves. If the concept of yields and yield curves is new to you, consider conducting further online research. A good starting point for your research would be the [U.S. Treasury website](https://home.treasury.gov){target="_blank"}, from where you sourced your data.
...
- (Optional: [Intermediate Data Visualization with ggplot2](https://app.datacamp.com/learn/courses/intermediate-data-visualization-with-ggplot2){target="_blank"})
- (Optional: [Skill Track: Text Mining with R](https://app.datacamp.com/learn/skill-tracks/text-mining-with-r){target="_blank"})
...
2. **DataCamp Courses**: Complete the DataCamp courses [Introduction to Data Visualization with ggplot2](https://app.datacamp.com/learn/courses/introduction-to-data-visualization-with-ggplot2){target="_blank"} and [Joining Data with dplyr](https://app.datacamp.com/learn/courses/joining-data-with-dplyr){target="_blank"} to learn how to visualize and merge data effectively. The optional courses, [Intermediate Data Visualization with ggplot2](https://app.datacamp.com/learn/courses/intermediate-data-visualization-with-ggplot2){target="_blank"} and [Skill Track: Text Mining with R](https://app.datacamp.com/learn/skill-tracks/text-mining-with-r){target="_blank"}, can further enhance your skills in data visualization and text mining.
